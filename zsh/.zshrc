### Fichier sous le controle d'Ansible 
### Ne pas modifier, voir : https://git.resel.fr/confs/ansible
...
alias md='mkdir'
alias rd='rmdir'
alias upgrade='emerge -uDU --keep-going --with-bdeps=y --ask @world'
alias grep="grep --color=auto" #De la couleur pour grep
...
atColor=$(tput setaf $(hostname | sum | awk -v ncolors=$(printf "%d\n" $(infocmp -1 | expand | sed -n -e "s/^ *colors#\(\(0x\)\?[0-9][0-9]*\),.*/\1/p")) 'ncolors>1 {print 1 + ($1 % (ncolors - 1))}'))

if [ "`id -u`" -eq 0 ]; then
  export PS1="%{[36;1m%}%T %{[31m%}%n%{$atColor%}@%{$hostnameColor%}%m %{[32m%}%~ %{$atColor%}%#%{[0m%} "
else
  export PS1="%{[36;1m%}%T %{[34m%}%n%{$atColor%}@%{$hostnameColor%}%m %{[32m%}%~ %{$atColor%}%#%{[0m%} "
fi
...
bindkey '^[Oc' emacs-forward-word                        # [Ctrl-RightArrow] - move forward one word
bindkey '^[Od' emacs-backward-word                       # [Ctrl-LeftArrow] - move backward one word

bindkey '^H' backward-kill-word  # [Control Delete]
bindkey "^[[3^" kill-word        # [Control Suppr]
# bindkey '^W' kill-region         # [Control W]
